...
double compute_global_fitness_randomized_short(int kin_node, int kout_g, int tm, int degree_node, double minus_log_total) {
	...
	return min(topologic, weight_part);
		...
		facts::facts(int a, double b, multimap<double, int>::iterator c, int d)  {internal_degree=a; minus_log_total_wr=b; fitness_iterator=c; degree=d; };
		facts::~facts(){};
		
		
		weighted_tabdeg::weighted_tabdeg(){};
		weighted_tabdeg::~weighted_tabdeg(){};
		
		int weighted_tabdeg::size() {return lab_facts.size();};

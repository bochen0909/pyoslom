...
oslom_module::oslom_module(int a, int b) { nc=1; kout_in=a; ktot_in=a; kout_out=b; ktot_out=b; };
oslom_module::~oslom_module(){};
	

	
internal_links_weights::internal_links_weights(int a, double b, int c, double d) { k_in=a; win=b; k_out=c; wout=d; };
internal_links_weights::~internal_links_weights(){};
...
oslomnet_louvain::oslomnet_louvain(): static_network(){};
...
void oslomnet_louvain::unweighted_favorite_of(const int & node, int & fi, int & kp_in, int & kop_in, int & kp_out, int & kop_out) {
	...
	for(int j=0; j<vertices[node]->inlinks->size(); j++)
		oslom::int_histogram(vertex_label[vertices[node]->inlinks->l[j]], M, vertices[node]->inlinks->w[j].first, 0);
	
	for(int j=0; j<vertices[node]->outlinks->size(); j++)
		oslom::int_histogram(vertex_label[vertices[node]->outlinks->l[j]], M, 0, vertices[node]->outlinks->w[j].first);

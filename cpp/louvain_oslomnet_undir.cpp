...
oslom_module::oslom_module(int a) {
	nc = 1;
	kout = a;
	ktot = a;
}
;
oslom_module::~oslom_module() {
}
;
...
oslomnet_louvain::oslomnet_louvain() :
		static_network() {
}
;
oslomnet_louvain::~oslomnet_louvain() {
...
void oslomnet_louvain::unweighted_favorite_of(const int & node, int & fi,
		int & kp, int & kop) {
	...
	for (map<int, int>::iterator itM = M.begin(); itM != M.end(); itM++) {
		...
		if (itM->first != vertex_label[node]) {
			...
			to_fit = topological_05(itM->second, itOM->second.kout,
					oneM - itOM->second.ktot, vertices[node]->stub_number);

		} else {

			kop = itM->second;
			int kout_prime = itOM->second.kout - vertices[node]->stub_number
					+ 2 * kop;
			to_fit = topological_05(itM->second, kout_prime,
					oneM - itOM->second.ktot + vertices[node]->stub_number,
...
void oslomnet_louvain::weighted_favorite_of(const int & node, int & fi,
		int & kp, int & kop) {
	...
	for (mapip::iterator itM = M.begin(); itM != M.end(); itM++) {
		...
		if (itM->first != vertex_label[node]) {
			to_fit = topological_05(itM->second.first, itOM->second.kout,
					oneM - itOM->second.ktot, vertices[node]->stub_number);
			to_fit *= double(dim - itOM->second.nc + 1)
					/ (min(dim - itOM->second.nc,
							itOM->second.kout / itM->second.first + 1) + 1);
			...
			to_fit = topological_05(itM->second.first, kout_prime,
					...
					vertices[node]->stub_number);
			to_fit *=
					double(dim - itOM->second.nc + 2)
							/ (min(dim - itOM->second.nc + 1,
									kout_prime / kop + 1) + 1);
...
void oslomnet_louvain::update_modules(const int & i, const int & fi,
		const int & kp, const int & kop) {
...
void oslomnet_louvain::single_pass_unweighted() {
	...
	for (deque<int>::iterator itd = vertex_order.begin();
			itd != vertex_order.end(); itd++) {
...
void oslomnet_louvain::set_partition_collected(deque<deque<int> > & ten2) {
	...
	for (int i = 0; i < dim; i++) {
		...
		pair<map<int, int>::iterator, bool> itm_bool = mems.insert(
				make_pair(vertex_label[i], mems.size()));
...
int oslomnet_louvain::collect_raw_groups_once(deque<deque<int> > & P) {
	...
	while (true) {
		...
		if (double(nodes_changed - previous_nodes_changed)
				> 1e-3 * previous_nodes_changed
				&& iteration > paras->max_iteration_convergence)
			stopper++;

		if (stopper == paras->max_iteration_convergence || nodes_changed == 0
				|| iteration == dim)
			break;
